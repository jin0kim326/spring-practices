...
//		testApplicationContext01();
	...
	private static void testApplicationContext02() {
		BeanFactory ac = new ClassPathXmlApplicationContext("com/douzone/container/config/user/applicationContext02.xml");
		...
		// 타입으로 빈 가져오기
		// 같은 타입의 빈이 두개 이상 있을때 타입으로 가져오면 에러 발생
		/* 해결방법
		 * 1. id + type
		 * 2. name + type
		 */
	...
	private static void testApplicationContext01() {
		BeanFactory ac = new ClassPathXmlApplicationContext("com/douzone/container/config/user");
	...
	private static void testBeanFactory01() {
		BeanFactory bf = new XmlBeanFactory ( new ClassPathResource("com/douzone/container/config/user"));
	...
	private static void testBeanFactory02() {
		...
		//Bean id가 자동으로 설정되지 않음 => 명시적 설정 필요  
		user1 = (User1)bf.getBean("user1");
		System.out.println(user1.getName());
	}
